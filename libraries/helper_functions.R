...
# Assume data_df$date are in UTC posix, no date overlaps
get_sensor_uptime <- function(
  ...
  print(start_post_outage_indices)
  print(end_post_outage_indices)
  ...
  print(before_outage_indices)
  print(after_outage_indices)
  ...
  ]] == (length(after_outage_indices) + 1)) {
    dates_after_outage[[length(dates_after_outage)]] <- last_expected_df_date
  }
  print(dates_before_outage)
  print(dates_after_outage)
...
uptime <- get_sensor_uptime(readr::read_csv(
